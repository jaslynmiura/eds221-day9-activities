...
mack_subset <- mack_creek_vertebrates %>% 
  select(SPECIES, LENGTH1, LENGTH2, SAMPLEDATE, SECTION) %>% 
  filter(SECTION == "CC") %>% 
  slice_head(n = 10)

datapasta::df_paste(mack_subset)
...
```{r}
# Example 2:
mack <- mack_creek_vertebrates %>% 
  clean_names() %>% 
  filter(species == "ONCL")

ggplot(data = mack, 
       aes(x = length1, y = weight, shape = sitecode, color = species)) +
         geom_point() +
         theme_minimal() +
...
mack_subset <- mack_creek_vertebrates %>% 
  select(SPECIES, LENGTH1, WEIGHT, SITECODE, SECTION) %>% 
  filter(SPECIES == "ONCL") %>% 
  slice_head(n = 10)
...
data.frame(
  stringsAsFactors = FALSE,
           ...
           LENGTH1 = c(58, 61, 89, 58, 93, 86, 107, 131, 103, 117),
